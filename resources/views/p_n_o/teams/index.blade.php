...
@section('title') Medical Teams @endsection
@section('styles')
...
@endsection
@section('location')
	<h1>Medical Teams - {{ config('app.name') }}</h1>
    <ol class="breadcrumb">
        <li>
            <a href="{{ route('home') }}">
                        ...
                        <tbody>
                            ...
                            @foreach($teams as $team)
                            	<tr>
                            		...
                            		<td class="text-center">{{ $team->team_members }}</td>
                            		<td><textarea rows="5" class="form-control" disabled style="background-color: #fff; border: none;">{{ $team->description }}</textarea></td>
                            		<td>{{ $team->status }}</td>
                            		<td>
                            			<div class="row">
                            				<div class="col-md-6">
                            					<a href="{{ route('teams.show', $team->id) }}" class="btn btn-xs btn-info pull-left">View Details</a>
                            				</div>
                            				<div class="col-md-6">
                            					<a href="{{ route('teams.edit', $team->id) }}" class="btn btn-xs btn-primary pull-right btn-block">Edit Record</a>
                            				</div>
                            			</div>
                            		</td>
                            	</tr>
        ...
        <div class="col-md-2 col-sm-8">
        	<div class="panel panel-danger filterable">
        		<div class="panel-body text-center">
    				<div class="row">
        				<a href="{{ route('teams.create') }}" class="btn btn-info btn-block">Add New</a>
        				<hr>
